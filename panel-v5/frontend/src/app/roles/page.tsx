        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
           ...
           <div className="glass group rounded-3xl border border-white/10 overflow-hidden hover:border-emerald-500/30 transition-all duration-300">
              <div className="p-8 space-y-6">
                 ...
                 <div>
                    <h3 className="text-2xl font-black tracking-tight">Super Administrator</h3>
                    <p className="text-sm text-neutral-500 mt-2">Unrestricted access to all modules and configurations.</p>
                 </div>

                 <div className="space-y-3">
                    <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Capabilities</p>
                    <div className="flex flex-wrap gap-2">
                       {['Full Root', 'Network Mgmt', 'User Control', 'Billing'].map(tag => (
                          <span key={tag} className="px-2 py-1 rounded-lg bg-emerald-500/5 border border-emerald-500/10 text-[10px] text-emerald-400/70 font-medium">
                             {tag}
                          </span>
                       ))}
                       <span className="px-2 py-1 rounded-lg bg-white/5 text-[10px] text-neutral-500 font-medium">+12 more</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-white/5 p-4 flex items-center justify-between border-t border-white/5">
                 <div className="flex items-center gap-2 text-neutral-500">
                    <Users size={16} />
                    <span className="text-xs font-bold">4 Users Assigned</span>
                 </div>
                 <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 text-emerald-400 text-xs font-black hover:bg-emerald-500 hover:text-white transition-all">
                    MANAGE PERMISSIONS
