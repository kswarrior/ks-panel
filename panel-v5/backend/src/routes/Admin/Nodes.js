...
router.get("/admin/nodes/overview", hasPermission('manage_nodes'), async (req, res, next) => {
...
router.get("/admin/nodes/create", hasPermission('manage_nodes'), async (req, res, next) => {
  if (req.query.legacy !== "true") return next();
...
router.get("/admin/nodes/node/:id/stats", hasPermission('manage_nodes'), async (req, res, next) => {
  if (req.query.legacy !== "true") return next();
  ...
  try {
    ...
    if (set[node.id] > 0) {
      ...
      if (req.query.deleteinstances === "true") {
        let delinstances = instances.filter(i => i.Node.id === node.id);
        instances = instances.filter(i => i.Node.id !== node.id);
        await db.set("instances", instances);
        for (const instance of delinstances) await db.delete(instance.Id + "_instance");
        ...
        try {
          await axios.get(`http://kspanel:${node.apiKey}@${node.address}:${node.port}/instances/purge/all`);
        } catch (apiError) {
...
router.post("/admin/nodes/configure", async (req, res) => {
  ...
  if (!configureKey) return res.status(400).json({ error: "Missing configureKey" });
  ...
  try {
    ...
    const fullConfig = {
      ...
      "ftp": { "ip": foundNode.ftp.ip, "port": foundNode.ftp.port },
      "ssl": {
...
router.get("/admin/nodes/node/:id/configure-command", hasPermission('manage_nodes'), async (req, res) => {
  ...
  try {
    ...
    res.json({ nodeId: id, configureCommand: configureCommand });
  } catch (error) {
...
router.post("/admin/nodes/node/:id", hasPermission('manage_nodes'), async (req, res) => {
  ...
  if (!node.name || !node.address || !node.port) return res.status(400).json({ error: "Missing required fields" });
...
router.post('/admin/nodes/overview/:id/allocations', hasPermission('manage_nodes'), async (req, res) => {
  ...
  if (!portsInput) return res.status(400).json({ error: 'Missing portsInput' });
  try {
    const ports = parsePorts(portsInput);
    const allocationsKey = `${id}_allocations`;
    let allocations = (await db.get(allocationsKey)) || [];
    const existingPorts = allocations.filter(a => a.ip === (ip || node.address)).map(a => a.port);
    const conflicts = ports.filter(p => existingPorts.includes(p));
    if (conflicts.length > 0) return res.status(409).json({ error: `Ports already allocated: ${conflicts.join(', ')}` });
    const newAllocs = ports.map(port => ({ id: uuidv4(), ip: ip || node.address, alias: alias || null, port, assignedTo: null }));
...
router.get('/admin/nodes/overview/:id/available-allocations', hasPermission('manage_nodes'), async (req, res) => {
  ...
  const available = allocations.filter(a => !a.assignedTo).map(a => ({ id: a.id, label: `${a.ip}:${a.port}${a.alias ? ` (${a.alias})` : ''}` }));
  res.json(available);
});
